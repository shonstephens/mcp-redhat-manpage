...
#
# Requires: podman, network access to registry.access.redhat.com
...
extract_version() {
  ...
  podman run --name "$container_name" "$image" bash -c "
    set -e
  ...
  local manfiles
  manfiles=$(podman start -a "$container_name" 2>/dev/null || true)

  if [ -z "$manfiles" ]; then
    # Re-run to get file list
    manfiles=$(podman exec "$container_name" find /usr/share/man -type f \( -name '*.gz' -o -name '*.[0-9]' -o -name '*.[0-9]p' \) 2>/dev/null | sort || true)
  fi

  # Use podman cp to extract the entire man directory
  podman cp "$container_name:/usr/share/man" "$tmpdir/man" 2>/dev/null || {
...
# Generate an index file listing all available man pages per version
generate_index() {
  local index_file="$MANPAGES_DIR/index.json"
  ...
  node -e "
    ...
    const manDir = '$MANPAGES_DIR';
    const index = {};

    for (const version of readdirSync(manDir)) {
      if (!version.startsWith('rhel')) continue;
      const dir = join(manDir, version);
      try {
        const files = readdirSync(dir).filter(f => f.endsWith('.txt'));
        index[version] = files.map(f => {
          const parts = f.replace('.txt', '').split('.');
          const section = parts.pop();
          const name = parts.join('.');
          return { name, section, file: f };
        });
      } catch (e) {}
    }
